...
def preprocess_columns(data):
    ...
    data["pretty_measure"] = [c.replace("complexity.", "").replace("_adjusted1", "").replace("_", "-") for c in
                              data.actual_measure]
    return data
...
df = pd.read_csv(resultspath / f'{tag}_export.csv')[
    ['lr', 'bias', 'datafile', 'env_split', 'actual_measure', 'only_bias__ignore_input', 'selected_single_measure',
     'bias.1', 'loss', 'weight', '_runtime', 'risk_max', 'risk_min', 'train_mse', 'risk_range', 'robustness_penalty']]
...
for row, env_split in enumerate(['all', 'lr', 'depth', 'width', 'train_size']):
    if exp_type == 'v1' and env_split != 'all':
        continue
    data = [(D(*x.name.split('__')), np.sqrt(np.load(x))) for x in
            Path('temp/single_network/risks').glob(f'*__{env_split}__{exp_type}__False.npy')]
    baseline = [(D(*x.name.split('__')), np.sqrt(np.load(x))) for x in
                Path('temp/single_network/risks').glob(f'*__{env_split}__{exp_type}__True.npy')]
    data[0][0], len(data)
    if sorting is None:
    ...
    for i in range(len(data)):
        ...
        plt.axhline(np.percentile(data[i][1], q=90) * points / maxx, color="magenta", zorder=2, linewidth=1.5,
                    linestyle="--", label='90th percentile')
        plt.axhline(np.mean(data[i][1]) * points / maxx, color='orange', zorder=2, linewidth=1.5, linestyle=":",
                    label='mean')
        plt.ylabel('')
        if i == 0:
