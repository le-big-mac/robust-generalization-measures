...
if flags.env_split == "all" and flags.exp_type in {'v2', 'v3'}:
    ...
    data_obs = [tuple(dict(ChainMap(*[i[ii][iii] for ii in range(len(hp_dict))])) for iii in range(len(i[0]))) for i in
                data_obs]
else:
...
def estimator(envs, model, seed=99999, model_type="rf", _idx=None, name=None):
    ...
    wandb.join()
    np.save(
        f'temp/single_network/risks/{flags.selected_single_measure}__{flags.env_split}__{flags.exp_type}__{flags.only_bias__ignore_input}.npy',
        risks.detach().numpy())
    return model
...
baseline_models = [estimator(data_obs[idx][0], MLP(), seed=r * 9 + idx, _idx=idx, name=name) for idx, name in
                   enumerate(cms)]
...
baseline_train_mses = [torch.mean((data_train[idx][1] - baseline_models[0](data_train[idx][0])) ** 2) for idx, _ in
                       enumerate(cms)]
