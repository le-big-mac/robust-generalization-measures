...
class BaseLogger(object):
    ...
    def log_generalization_gap(self, state: State, train_acc: float, val_acc: float, train_loss: float, val_loss: float,
                               all_complexities: Dict[ComplexityType, float]) -> None:
        self.log_metrics(
            state.global_batch,
            {
                'generalization/error': train_acc - val_acc,
                'generalization/loss': train_loss - val_loss,
                'complexity/MARGINS': wandb.Histogram(all_complexities["margins"]),
                **{'complexity/{}'.format(k.name): all_complexities.get(k, None) for k in ComplexityType}
            })
...
class WandbLogger(BaseLogger):
    def __init__(self, tag: Optional[str] = None, hps: Optional[dict] = None, group: Optional[str] = None):
        wandb.init(project='margin_hist', config=hps, tags=[tag], group=group)
